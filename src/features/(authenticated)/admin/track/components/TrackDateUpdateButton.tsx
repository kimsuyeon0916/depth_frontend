'use client'

import React, { useState, useTransition } from 'react'
import { SquarePen } from 'lucide-react'
import Button from '@/components/ui/button/Button'
import { updateTrackAction } from '@/features/(authenticated)/admin/track/actions/updateTrack'
import { useRouter } from 'next/navigation'
import Input from '@/components/ui/Input'

const toDateInput = (v: string | Date | null | undefined) => {
  if (!v) return ''
  if (typeof v === 'string') return v.slice(0, 10)
  return v.toISOString().slice(0, 10)
}

export default function TrackDateUpdateButton({
  trackId,
  startDate,
  endDate,
}: {
  trackId: number
  startDate: string | Date
  endDate: string | Date
}) {
  const router = useRouter()
  const [isUpdate, setIsUpdate] = useState(false)
  const [sd, setSd] = useState(() => toDateInput(startDate))
  const [ed, setEd] = useState(() => toDateInput(endDate))
  const [isPending, startTransition] = useTransition()

  const open = () => {
    setSd(toDateInput(startDate))
    setEd(toDateInput(endDate))
    setIsUpdate(true)
  }

  const onCancel = () => {
    setSd(toDateInput(startDate))
    setEd(toDateInput(endDate))
    setIsUpdate(false)
  }

  const onSubmit = () => {
    startTransition(async () => {
      try {
        await updateTrackAction({ trackId, startDate: sd, endDate: ed })
        setIsUpdate(false)
        router.refresh()
      } catch (e) {
        alert(e instanceof Error ? e.message : '수정 실패')
      }
    })
  }

  const unchanged = sd === toDateInput(startDate) && ed === toDateInput(endDate)

  return (
    <div>
      <button
        type="button"
        onClick={open}
        className="rounded-full p-2 opacity-0 transition-opacity duration-150 group-hover:opacity-100 hover:bg-gray-200"
        aria-label="update"
      >
        <SquarePen size={16} />
      </button>

      {isUpdate && (
        <div className="absolute inset-0 z-10 flex items-center gap-2 rounded-md bg-white px-4">
          <Input
            type="date"
            name="startDate"
            id={`startDate-${trackId}`}
            required
            value={sd}
            onChange={(e) => setSd((e.target as HTMLInputElement).value)}
            className="h-9 text-sm"
          />

          <Input
            type="date"
            name="endDate"
            id={`endDate-${trackId}`}
            required
            value={ed}
            onChange={(e) => setEd((e.target as HTMLInputElement).value)}
            className="h-9 text-sm"
          />

          <div className="ml-auto flex justify-end gap-2">
            <button
              type="button"
              onClick={onCancel}
              disabled={isPending}
              className="h-9 rounded-md border border-gray-300 px-3 text-sm hover:bg-gray-50 disabled:opacity-50"
            >
              취소
            </button>

            <Button
              variant="add"
              type="button"
              className="h-9 px-3 text-sm"
              onClick={onSubmit}
              disabled={isPending || unchanged || !sd || !ed}
            >
              {isPending ? '수정중...' : '수정'}
            </Button>
          </div>
        </div>
      )}
    </div>
  )
}
